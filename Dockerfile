...
ENV VPHP "$phpversion"
RUN apk add php${VPHP}-fpm $(if test ${VPHP} -eq 5; then echo $php5pkgs; elif test ${VPHP} -eq 7; then echo $php7pkgs; fi)
RUN sed -i 's/^listen *=.*/listen = 9000/' /etc/php7/php-fpm.d/www.conf
RUN echo "catch_workers_output = yes" >> /etc/php7/php-fpm.d/www.conf
RUN echo "access.log = /proc/1/fd/1" >> /etc/php7/php-fpm.d/www.conf
RUN echo "php_flag[display_errors] = on" >> /etc/php7/php-fpm.d/www.conf
RUN echo "php_admin_value[error_log] = /proc/1/fd/2" >> /etc/php7/php-fpm.d/www.conf
RUN echo "php_admin_flag[log_errors] = on" >> /etc/php7/php-fpm.d/www.conf
#RUN echo "error_log = /proc/1/fd/2" >> /etc/php7/php-fpm.conf
RUN echo 'include_path = ".:/usr/share/php7:/usr/share/php7/PEAR"' >> /etc/php7/php.ini
RUN mkdir /run/php
